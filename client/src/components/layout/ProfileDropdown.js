        ...
        <div className="absolute inset-y-0 right-0 flex items-center md:static md:inset-auto md:ml-8 md:pr-0">
            <div className="relative">
                ...
                <div className={`origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none ${toggleDropdown ? 'is-open' : 'is-closed'}`} role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabIndex="-1">
                    ...
                    <button onClick={onLogout} className="block px-4 py-2 text-sm text-gray-700 focus:outline-none" role="menuitem" tabIndex="-1" id="user-menu-item-1">Sign out</button>
                </div>
            </div>
        </div>
    )
