import { arrayToUpdate, db, storage } from 'database';
 ...
 */
export const deleteRecipe = (recipeId) => db.collection('recipes').doc(recipeId).delete();
...
export const getUserMealPlan = (userId) =>
    db.collection('meal-plans').where('ownerId', '==', userId).get();
 ...
 * @param {String} recipeId The ID of the recipe.
 */
export const getRecipe = (recipeId) => db.collection('recipes').doc(recipeId).get();
...
export const getRecipeSnapshot = (slug, cb) =>
    db.collection('recipes').where('slug', '==', slug).onSnapshot(cb);
...
export const getMealPlanSnapshot = (userId, cb) =>
    db.collection('meal-plans').where('ownerId', '==', userId).onSnapshot(cb);
...
export const updateMealPlanDate = (recipe, newDate) =>
    db
        ...
        .get()
        .then((querySnapshot) => {
            querySnapshot.docs[0].ref.get().then((snapshot) => {
                const recipes = snapshot
                    .data()
                    .recipes.map((item) =>
                        item.id === recipe.id ? { ...item, date: newDate } : item,
                    );
